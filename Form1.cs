...
public partial class Form1 : Form
{
    ...
    {
        cityTextBox = new TextBox();//declaring all the labels and textboxes
        ...
        clearOptions(); // Reset all options to their defaults
    ...
    void getWeather()
    {
        ...
        {
            string url = String.Format($"http://api.openweathermap.org/data/2.5/weather?q={cityTextBox.Text}&appid={apiKey}&units=metric");//url for openweather api
            ...
            using (HttpClient client1 = new HttpClient())
            {
                try//to make sure of a succesful connection
                {
                    ...
                    if (weatherJson["cod"].ToString() == "200")//200 means the request was successful
                    {
                ...
                {
                    Console.WriteLine("Error: " + ex.Message);//writes error message to the console
                    clearOptions(); // Clear the TextBox from any previous input
                }
            }
    ...
    DateTime convertDateTime(long milisec)//convert milliseconds into proper date time format
    {
